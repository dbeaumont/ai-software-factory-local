...
/** Exact URI allow-list with fail-closed network-address validation and DNS pinning. */
public final class SecureUriPolicy {
    ...
    public URI requireAllowed(URI candidate) {
        ...
        validateNetworkTargetSyntax(value);
        Set<String> current;
        try {
    ...
    public static void validateNetworkTargetSyntax(URI uri) {
        ...
        try {
            if (host.matches("[0-9.]+") || host.contains(":")) {
                if (forbidden(InetAddress.getByName(host))) {
                    throw new SecurityException("Outbound network target is forbidden");
                }
            }
        } catch (SecurityException failure) {
    ...
    public interface Resolver {
        java.util.List<InetAddress> resolve(String host) throws Exception;
    }
}
