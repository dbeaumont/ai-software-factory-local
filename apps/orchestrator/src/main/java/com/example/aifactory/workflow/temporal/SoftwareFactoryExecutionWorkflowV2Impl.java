...
public final class SoftwareFactoryExecutionWorkflowV2Impl extends ProductionExecutionWorkflowRuntime
    ...
    public SoftwareFactoryWorkflow.Result run(Request request) {
        ...
        SoftwareFactoryWorkflow.Request executionRequest = request.hierarchicalRequest();
        bindSource(source, executionRequest, resolved);
        ...
        if ("HUMAN_TRIAGE".equals(routingDecision.selectedPath())) {
            recordRoutingTriage(source, executionRequest, resolved);
            phase = "GATE_REJECTED:routing";
            return new SoftwareFactoryWorkflow.Result(request.taskId(), request.attemptId(),
                    resolved.sourceCommit(), phase, List.of(
        ...
        SoftwareFactoryWorkflow.Result result = execute(executionRequest, resolved, true);
        phase = result.status();
        return result;
    }
