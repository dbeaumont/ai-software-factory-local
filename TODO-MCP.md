...
- [ ] **MCP-054** — Passer le Planner en `MCP_SHADOW` : comparer le contexte actuel à un contexte reconstruit via outils, sans changer le prompt servi. _(Mode, métriques et générateur de rapport disponibles ; smoke réel réussi le 2026-08-31 après correction de l'image Git et du contrat snake_case. Échantillon validé : 4/20 tâches, campagne multi-technologies encore requise.)_
- [ ] **MCP-055** — Mesurer couverture des fichiers utiles, citations valides, octets/tokens, latence et impact sur la qualité du plan. _(Sur 4 passages : couverture 100 %, citations au format attendu 100 %, mais 15 856 caractères MCP contre 11 160 directs ; le critère de réduction n'est pas validé. Le modèle local fonctionne après correction du bootstrap et du healthcheck LiteLLM, mais les deux plans évalués s'arrêtent en `NEEDS_CLARIFICATION` sur le chemin direct : baseline Planner à stabiliser avant de mesurer une non-régression MCP.)_
