...
- [x] **A2A-089 — Remplacer les child workflows d'agents.** Conserver les child workflows de contrôle nécessaires
  ...
  construit l'enveloppe `a2a-envelope-v1` complète autour d'une référence Evidence, appelle `dispatchTask`,
  attend signal/réconciliation, puis valide l'artefact final ; l'ancienne implémentation
...
- [x] **A2A-090 — Maintenir la revue indépendante.** Adresser `independent-reviewer` par A2A avec les mêmes digests
  ...
  production est désormais dédié à `independent-reviewer` et ne transmet, dans une enveloppe validable unique,
  que les références Evidence du patch
...
- [x] **A2A-146 — Créer les alertes.** Détecter absence de poller, agent non prêt, carte invalide, taux d'échec,
  backlog, tâche bloquée, notification en retard, collision d'idempotence et divergence d'état. _(Neuf règles
...
- [x] **A2A-162 — Exécuter le TCK A2A officiel.** Tester chaque opération et capacité déclarée avec la version
...
- [x] **A2A-171 — Tests de non-régression pipeline.** Vérifier que le mode `PIPELINE`, s'il reste supporté comme
  mode métier, ne réintroduit aucun appel direct d'agent ni sélection de transport. _(Terminé le 2026-09-07 :
  les cinq opérations agentiques du pipeline passent par des tâches et preuves A2A contractuelles ; 22 tests
  ciblés passent et un garde source interdit agents in-process, appel LLM direct et sélecteur de transport ; preuve :
  `docs/evidence/a2a/A2A-171-PIPELINE-NON-REGRESSION.md`.)_
