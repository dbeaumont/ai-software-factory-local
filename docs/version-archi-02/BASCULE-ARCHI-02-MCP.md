...
- [x] **MCP-054** — Passer le Planner en `MCP_SHADOW` : comparer le contexte actuel à un contexte reconstruit via outils, sans changer le prompt servi. _(Campagne cloud de 20 tâches exécutée le 2026-09-01 après publication des trois fixtures : 20/20 reconstructions Context shadow réussies, sans modifier le prompt servi. Résultats minimisés et rapport : `MCP-context-shadow-campaign-20260901-144626.jsonl` et `MCP-shadow-20260901-144626.md`.)_
- [x] **MCP-055** — Mesurer couverture des fichiers utiles, citations valides, octets/tokens, latence et impact sur la qualité du plan. _(Gate technique `GO` : campagne `20260901-205843` à 20/20 contrats valides, couverture 92 %, citations 100 %, réduction de contexte 29,2 % et revue sémantique conforme. Deux troncatures `length` ont été récupérées par l'unique retry à 2 400 tokens ; aucun refus ou filtre. MCP-056 peut démarrer en canary Planner.)_
- [x] **MCP-056** — Basculer Planner puis Developer/PatchRepair sur `MCP_ACTIVE`, rôle par rôle. _(Deux canaries successifs validés : Planner seul à 20/20 plans, puis `planner,developer,patch-repair` avec 20/14/6 collectes MCP actives attendues, zéro fallback direct et zéro contrat Planner invalide ; rapports `MCP-active-20260901-211328.md` et `MCP-active-20260901-212521.md`.)_
- [x] **MCP-057** — Remplacer `RepositoryContextService.collect()` par l'adaptateur MCP après une période de stabilité. _(Collecteur direct et gateway supprimés après deux canaries actifs ; `McpRepositoryContextService` est l'unique provider, les anciens modes et rôles hors allow-list échouent fermé. Voir `docs/mcp/MCP-057-retrait-collecteur-direct.md`.)_
...
- [x] **MCP-076** — Implémenter `run_tests` par profil ; conserver l'usage explicite d'Artifactory et interdire l'egress non requis. _(Maven/Gradle imposent le miroir serveur — Maven Central via proxy pour le dev Internet, Artifactory avec jeton lecteur en environnement intégré —, npm impose son registre, Node.js LTS est présent et les jobs rejoignent uniquement un réseau interne avec proxy allow-listé.)_
- [x] **MCP-077** — Implémenter temporairement `run_quality` et `run_security` à parité avec `SandboxService`. _(Profils immuables qualifiés en conditions réelles : SonarQube `PASSED` avec preuve complète, Syft/Trivy `PASSED` avec SBOM et base Trivy téléchargée via le proxy allow-listé. Rapport : `docs/mcp/MCP-077-parite-qualite-securite.md`.)_
...
- [x] **MCP-087** — Exécuter `SandboxService` et MCP en shadow ; comparer exit codes, diff stat, tests, qualité, SBOM et Trivy. _(Campagne isolée sur deux clones du même commit : diff stats identiques, 3/3 tests réussis et SBOM sémantiquement identiques. Les échecs DNS du chemin historique pour Sonar/Trivy sont une divergence de durcissement acceptée : MCP réussit via le proxy allow-listé. Rapport : `docs/mcp/MCP-087-campagne-shadow-sandbox.md`.)_
- [x] **MCP-088** — Basculer chaque opération sur MCP derrière un feature flag indépendant. _(La liste `AI_FACTORY_MCP_SANDBOX_ACTIVE_OPERATIONS` pilote séparément `validate_patch`, `apply_patch`, `run_tests`, `run_quality` et `run_security`; le déploiement local active explicitement les cinq opérations.)_
...
- [x] **MCP-111** — Créer un registre de dépôts autorisés ; accepter un `repository_id`, jamais des credentials ou une URL arbitraire. _(Registre JSON v1 des trois fixtures, validation stricte ID/owner/name/clone path/branches et résolution fail-closed ; test de refus d'une URL arbitraire.)_
- [x] **MCP-112** — Implémenter `get_repository` et `resolve_revision` avec contrôles d'organisation, dépôt et branche. _(Deux outils MCP read-only résolvent uniquement le registre serveur, contrôlent acteur/deadline/branche et ne retournent que des métadonnées sans secret ou un SHA Git complet.)_
- [x] **MCP-113** — Définir une preuve d'approbation vérifiable contenant tâche, tentative, source SHA, patch digest, décision, approbateur et expiration. _(Schéma JSON v1 fermé et preuve HMAC-SHA-256 canonique liant tous les champs, limitée à 24 h ; altération et expiration couvertes par tests.)_
- [x] **MCP-114** — Implémenter `create_draft_pull_request` comme commande métier atomique et idempotente. _(Commande MCP unique : validation registre/preuve/digest, staging contrôlé, commit, push authentifié par `GIT_ASKPASS` sans credential dans l'URL, puis création d'une PR brouillon structurée. La persistance de rejeu est finalisée par MCP-115.)_
- [x] **MCP-115** — Garantir qu'un retry retrouve la PR existante au lieu de créer une deuxième branche/PR. _(Résultats persistés atomiquement par clé+empreinte, rejeu local sans effet ; après timeout ambigu, recherche Gitea par dépôt/base/head avant toute nouvelle écriture. Une clé réutilisée avec d'autres paramètres est refusée.)_
- [x] **MCP-116** — Vérifier source SHA, branche cible protégée, digests de preuves et approbation avant toute écriture. _(Avant appel du backend : HEAD Git égal au SHA approuvé, branche dans le registre, patch signé, six preuves obligatoires présentes et conformes à leurs SHA-256, preuve HMAC valide et liée à la livraison.)_
- [x] **MCP-117** — Conserver l'exclusion des artefacts `.ai-plan.md`, `changes.patch`, `.ai-review.md` et `.ai-factory` du commit, avec test explicite. _(Staging `git add -A` suivi d'un reset explicite des quatre artefacts ; test Git réel vérifiant que seul le fichier produit apparaît dans le commit.)_
- [x] **MCP-118** — Refuser force push, merge, suppression, écriture sur branche de base et changement de remote. _(Seulement trois outils publiés, aucun outil Git générique/merge/delete ; refspec sans `+` vers une branche `ai-factory/*` distincte de la base et remote reconstruit depuis le registre serveur, couverts par tests.)_
- [x] **MCP-119** — Produire un événement d'audit avant/après écriture avec PR ID/URL, commit et clé d'idempotence. _(Journal JSONL synchronisé et forcé sur disque : événement `BEFORE_WRITE` avec source/patch, puis `AFTER_WRITE` avec commit, PR ID/URL et clé d'idempotence ; aucun secret journalisé.)_
...
- [x] **MCP-142** — Déplacer l'interprétation du quality gate hors de `TaskService.requireQualityGate()` vers `evaluate_quality_gate`. _(`TaskService` délègue à `assurance.evaluate_quality_gate`; le serveur reconnaît explicitement les statuts Sonar connus et retourne `INDETERMINATE` pour preuve/format technique incomplet. L'orchestrateur valide le schéma normalisé et n'accepte que `PASSED`.)_
- [x] **MCP-143** — Normaliser les findings Trivy/Sonar avec sévérité, règle, composant/fichier, preuve et recommandation. _(`assurance.normalize_findings` harmonise les vocabulaires Sonar/Trivy vers UNKNOWN/LOW/MEDIUM/HIGH/CRITICAL, borne tous les champs, exige règle/composant/preuve/recommandation et produit résumé, verdict et référence de preuve normalisés.)_
- [x] **MCP-144** — Rendre `INDETERMINATE` bloquant pour timeout, format inconnu, scanner absent ou preuve manquante. _(`evaluate_quality_gate` et `normalize_findings` produisent `INDETERMINATE` sur exécution/preuve incomplète ou format/sévérité inconnue ; `assurance.evaluate_policy` exige tests, qualité, sécurité et SBOM avec digests valides, et ne retourne `ALLOW` que si les quatre verdicts sont `PASSED`.)_
- [x] **MCP-145** — Scaffolder `evidence-mcp` avec stockage local immuable par tentative et digests vérifiés. _(Serveur non-root isolé sur 8095 ; `evidence.store` vérifie SHA-256 avant `CREATE_NEW`, rejoue idempotemment le même contenu et namespace les artefacts par tâche/tentative/type. Aucun workspace ni secret runtime n'est monté.)_
- [x] **MCP-146** — Enregistrer plan, patch, métadonnées, logs de tests, Sonar, SBOM, Trivy, review et approbation dans un manifeste unique. _(`evidence.create_manifest` exige exactement neuf références déjà stockées dans la même tâche/tentative, vérifie le digest du patch et la liaison de la décision de politique, puis persiste un manifeste canonique immuable et idempotent.)_
- [x] **MCP-147** — Ajouter rétention, classification, chiffrement et contrôle d'accès par type de preuve. _(Politique serveur par type : INTERNAL/CONFIDENTIAL et 90/180/365 jours ; écriture réservée au workflow, purge à l'initialisation, AES-256-GCM avec AAD tâche/tentative/type/digest avant stockage. En POC la clé est dérivée avec séparation de domaine ; clé Secret Manager dédiée prévue MCP-217.)_
...
- [x] **MCP-170** — Vérifier que LiteLLM et chaque modèle supporté conservent fidèlement noms, schémas, IDs et résultats d'appels d'outils. _(Contrat OpenAI-compatible qualifié pour le modèle logique `factory-code-cloud` : les noms MCP canoniques sont projetés vers des alias de transport conformes puis résolus côté hôte, le JSON Schema, `tool_call.id`, les arguments et la corrélation du résultat sont conservés ; tout alias ou champ stable inconnu, absent ou altéré est rejeté.)_
...
- [x] **MCP-172** — Appliquer la matrice de permissions côté hôte avant chaque appel ; ne jamais faire confiance au nom de rôle renvoyé par le modèle. _(`AgentToolLoop` reçoit une identité/rôle fixés par le workflow et consulte une matrice deny-by-default immédiatement avant chaque exécution. Aucun rôle ne figure dans les tours du modèle ; rôle inconnu, outil inconnu ou outil interdit produisent `tool_denied` sans invoquer le serveur.)_
- [x] **MCP-173** — Limiter d'abord Planner et Reviewer aux outils de lecture `context.*` et ressources de preuves autorisées. _(Matrice v1 alignée côté hôte : Planner dispose de `context.*` et `evidence.get_summary`; Reviewer ajoute uniquement `evidence.read`, dont le serveur impose déjà rôle, motif, cloisonnement tâche et audit. Stockage, manifeste, sandbox et livraison restent interdits aux deux rôles.)_
- [x] **MCP-174** — Garder `validate_patch`, `apply_patch`, tests, scans et livraison exclusivement pilotés par le workflow pendant ce lot. _(La surface d'effets du lot 6 est déclarée une seule fois côté hôte : validation/application du patch, tests, quality/security scan, annulation et création de PR ne sont accordés qu'au rôle `workflow`. Un test exhaustif prouve leur refus pour Planner, Developer, Patch-repair, Tester et Reviewer.)_
- [x] **MCP-175** — Ajouter confirmation/policy gate pour toute future capacité à effet ; afficher clairement outil, arguments sûrs et impact dans l'UI. _(Une `PendingEffect` structurée expose l'outil, uniquement des arguments affichables sans secret, l'impact, la décision de policy et l'obligation de confirmation. L'approbation refuse toute action absente/non autorisée ; l'UI présente ces éléments avant le bouton qui déclenche la PR.)_
- [x] **MCP-176** — Détecter boucles, appels répétés, fan-out excessif et explosion de contexte. _(En complément du maximum de tours, l'hôte borne les appels par tour, compte les appels identiques par nom+arguments et mesure le contexte avant le modèle et après chaque résultat. Les coupures stables `fan_out`, `repeated_call` et `context_limit` interviennent avant tout appel supplémentaire.)_
- [x] **MCP-177** — Filtrer les résultats d'outils comme données non fiables et conserver la séparation des prompts système. _(La boucle ajoute une garde système hôte immuable, conserve exactement un message `system`, et encapsule chaque résultat dans une frontière `untrusted_tool_result trust="none"`. Les fermetures de frontière injectées et le nom d'outil sont échappés ; aucun résultat n'est concaténé au prompt système.)_
- [x] **MCP-178** — Tester prompt injection dans code, ticket, log, nom d'outil, description d'outil et résultat MCP. _(Suite paramétrée sur six vecteurs : code, ticket et log ne peuvent fermer leur enveloppe ; les noms d'outils hors convention sont rejetés ; les descriptions sont bornées et marquées métadonnées non fiables ; les résultats ne peuvent fermer leur frontière. Les tests vérifient les séparations, indépendamment du comportement du modèle.)_
- [x] **MCP-179** — Tester un serveur compromis : outil ajouté dynamiquement, schéma modifié, réponse surdimensionnée, URI externe et instruction malveillante. _(Simulation hôte automatisée : catalogue divergent déclaré `INCOMPATIBLE`, schéma de réponse fermé, limite d'octets avant interprétation, URI de preuve limitée à `evidence://`, et instruction conservée dans l'enveloppe non fiable. Dossier : `docs/mcp/MCP-179-serveur-compromis.md`.)_
- [x] **MCP-180** — Lancer une évaluation A/B sur la suite de référence : réussite au premier patch, réparations, tests, acceptation humaine, tokens, durée et coût. _(Campagne cloud autorisée exécutée sur 20 cas appariés ; verdict `REJECTED` : tests 10 % → 0 % et tokens moyens +28,19 %, sans régression sécurité. L'acceptation humaine est correctement séparée de la review IA et reste à 0 % ; la télémétrie coût fournisseur est absente. Le mode agentique reste désactivé ; rapport : `docs/mcp/MCP-180-rapport-campagne-20260902.md`.)_
- [x] **MCP-181** — N'activer un rôle en production que si les seuils de qualité et de sécurité définis au lot 0 sont atteints. _(Démarrage fail-closed : aucun rôle actif par défaut ; Planner/Reviewer seulement sont acceptés, et uniquement avec verdict `QUALIFIED`, au moins 20 cas appariés et gate sécurité réussi. Toute combinaison incomplète ou tout rôle non prévu empêche le démarrage.)_
- [x] **MCP-182** — Ajouter un kill switch global et par serveur/outil/rôle, utilisable sans redéploiement mais réservé à l'exploitation. _(Fichier de contrôle relu avant chaque appel, sans API d'écriture applicative : coupure globale, serveur, outil ou rôle, révision obligatoire et fail-closed si fichier présent invalide. Le client MCP déterministe et la matrice agentique consultent ce contrôle ; procédure dans `docs/mcp/MCP-182-kill-switch.md`.)_
