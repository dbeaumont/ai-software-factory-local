...
Le rapport horodaté est créé dans `docs/evidence/mcp/baselines/`. Il contient les mesures agrégées et un extrait limité aux métriques MCP shadow, sans contenu de dépôt ni secret.
...
La revue qualitative et la recommandation de gate sont consignées dans [`MCP-055-revue-qualite-plans.md`](../../../architecture/mcp/MCP-055-revue-qualite-plans.md).
...
La campagne autorisée `20260901-154935` a qualifié cette reprise sur les 20 scénarios. Le compteur de retry vaut 1 : `CTX-011` a produit deux contrats successifs sans champ `status` et reste le seul plan invalide. Les 19 autres contrats sont valides, dont cinq décisions `NEEDS_CLARIFICATION` correctement non rejouées. L'échec résiduel s'est déplacé de `CTX-020` lors de la campagne précédente vers `CTX-011`, ce qui caractérise un défaut intermittent plutôt qu'une incompatibilité stable d'un cas donné. Le résultat reste 19/20 et ne lève pas le `NO-GO`.

Après ajout de la classification `finish_reason`, la campagne `20260901-205843` atteint 20/20 contrats valides. Deux premières réponses ont été explicitement classées `length` puis récupérées par l'unique seconde tentative à 2 400 tokens ; aucun refus ni filtre de contenu n'a été observé. La revue transitoire des résumés, fichiers impactés et tests confirme la cohérence sémantique des 20 plans. Le gate technique MCP-055 est donc satisfait et autorise la préparation du canary MCP-056.
