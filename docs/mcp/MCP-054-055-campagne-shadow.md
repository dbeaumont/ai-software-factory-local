...
La chaîne LLM locale a été remise en service en installant réellement `qwen2.5-coder:7b`. Le healthcheck LiteLLM utilise désormais `/health/liveliness` au lieu de `/health`, qui déclenchait des générations de contrôle concurrentes, et le timeout Ollama est porté à 600 secondes. Deux plans ont ensuite terminé, mais la baseline directe les a classés `NEEDS_CLARIFICATION`, y compris avec une exigence détaillée. Ce défaut de qualité du Planner empêche encore d'atteindre Developer/Sandbox et doit être distingué d'une régression MCP : en shadow, le Planner reçoit toujours le contexte direct.
